...
```{r augustus_name_count_line, echo = F}
babynames %>%
...
**Answer:** The movie "The Fault in Our Stars" was released in 2014.

## Plausible Explanation: Release of the movie "The Fault in Our Stars"

```{r augustus_tfos_annotated_line, echo = F}
babynames %>%
  filter(name == "Augustus") %>%
  filter(year >= 2000) %>%
ggplot() +
  ...
  geom_text(aes(x = 2009, y = 600, label = "The Fault in Our Stars"), color = "red", hjust = 0)
```
...
```{r augustus_name_count_line_labeled, echo = F}
babynames %>%
  filter(name == "Augustus") %>%
ggplot() +
  geom_line(aes(x = year, y = n)) +
...
```{r logical_scalar_ge_example}
x <- 1
x >= 2
```

```{r logical_vector_ge_example}
x <- c(1,2,3)
x >= 2
```
...
*Hint:* Don't forget about `ggplot()`'s pipe operator analogue `+`

## Pipes

```{r pipeline_plot_augustus}
babynames %>%
...
**Answer:** Using the `group_by()` function (coming up next).
...
```{r distinct_names_time_series, echo = F}
babynames %>%
  group_by(year, sex) %>%
...
```{r top10_names_share_time_series, echo = F}
babynames %>%
  group_by(year, sex) %>%
  arrange(prop) %>%
  top_n(10, wt = prop) %>%
...
```{r flash_in_pan_facets_examples, echo = F}
babynames %>%
...
```{r flash_in_pan_identify_and_plot, echo = F}
babynames_flash <-
  babynames %>%
  group_by(name, sex) %>%
...
```{r names_switch_facets_examples, echo = F}
babynames %>%
