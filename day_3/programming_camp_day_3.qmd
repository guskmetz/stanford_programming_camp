...
**Hint:** View the files in the `day_3.R` file.
...
```{r table1_cases_over_time_plot, echo = F}
...
**A:** `pivot_wider()` converts the column names to strings. This is fixed in the `day_3.R` file.
...
**Answer:** The column names are strings, but `pivot_longer()` expects them to be numbers because of how the list is specified. This is fixed in the `day_3.R` file.
...
**Activity:** Follow along with the code in the `day_3.R` file.
...
**Q:** How do you load the `nycflights13` package?
...
**Activity:** Do the below fact-finding activity.
...
Explore a bit: (**Hint:** Use the `view()` function to directly view the data.)
...
**Activity:** Produce the following chart. What do the underlying data look like?

```{r carrier_cancel_fraction_by_code_plot, echo = F}
flights %>%
  group_by(carrier) %>%
...
## My solution (yours may be different)

```{r carrier_cancel_fraction_by_code_plot_example, eval = F}
flights %>%
  group_by(carrier) %>%
  summarise(frac_can= sum(is.na(arr_delay)) / n()) %>%
  filter(frac_can > 0) %>%
...
```{r carrier_cancel_fraction_by_airline_plot, echo = F}
flights %>%
...
```{r build_nyc_flights_daily_summary}
nyc_flights <-
  flights %>%
...
```{r build_nyc_weather_and_join_delays}
nyc_weather <-
  weather %>%
...
```{r dep_delay_vs_precip_lm_plot, echo = F, fig.height = 4}
nyc_weather_delays %>%
  left_join(airlines, by = "carrier") %>%
